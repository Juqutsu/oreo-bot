...
1. **Auto-defer:** `index.js` defers **every** slash command **ephemerally** before `execute()` — except `/announcement` and any `/config … edit` subcommand (`getSubcommand(false) === 'edit'`, matches through groups). **Any new command that calls `showModal()` must be added to this exemption list** or it will crash (can't show modal after defer).
