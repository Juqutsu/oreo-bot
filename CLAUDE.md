...
13. **`/announcement` lifecycle** (`src/commands/announcement.js`, `src/interactions/announcement.js`, `src/announcements.js`): subcommands `create`/`edit`/`delete`/`list`, all `requiredTier: 'moderator'` — every moderator (not just the original author) may edit or delete any announcement; attribution is via the modlog embed (`edited_by` column), not an author-only gate. `create` and `edit` open a modal (title/description/image-url) whose submit renders an **ephemeral preview** (embed + Posten/Bearbeiten/Abbrechen/✏️ buttons) rather than posting immediately.
    ...
    - The "✅ Posten"/"✅ Übernehmen" button synchronously sets `session.posting = true` **before** any `await`, closing the double-click race where two near-simultaneous clicks both pass the session/user checks — the loser gets "⏳ Wird bereits gepostet …" instead of double-posting/double-writing.
    ...
    - Known gap, not a bug in practice: the delete-confirmation buttons (`delconfirm:yes/no`) have no equivalent synchronous claim flag — a double-click race there is possible in theory but harmless at worst (a duplicate "already deleted" no-op or a duplicate delete-modlog line), unlike the preview post race which would double-post to the channel.
...
- Minor: `/announcement`'s delete-confirmation buttons (`announcement:delconfirm:yes/no`) have no synchronous double-click claim flag, unlike the preview post buttons (see invariant 13) — a double-click race is possible in theory but harmless at worst (duplicate "already deleted" no-op or duplicate delete-modlog line, never a duplicate channel post).
